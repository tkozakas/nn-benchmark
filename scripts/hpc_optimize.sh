...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      echo "Usage: ./hpc_optimize.sh --user USERNAME [OPTIONS]"
      echo ""
...
JOBID=$($SSH_CMD "cd $REMOTE_DIR && mkdir -p logs && sbatch --parsable scripts/run_optimize.sh \
  $ARCHITECTURE $EPOCHS $PATIENCE $N_TRIALS")
echo "Job submitted with ID: $JOBID"

LOGFILE="logs/optimize_${JOBID}.out"
...
echo ""
echo "=== Done! ==="
echo "Log file: $LOCAL_DIR/logs/optimize_${JOBID}.out"
